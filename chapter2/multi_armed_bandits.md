...
为了简化表示法，我们专注于单一动作。让 $R_i$ 现在表示在第i次选择*此动作* 之后收到的奖励， 并且让 $Q_n`表示在其被选择 :math:`n-1$次之后其动作价值的估计，我们现在可以简单地将其写为
