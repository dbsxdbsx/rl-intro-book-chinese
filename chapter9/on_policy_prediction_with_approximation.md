...
​	虽然半梯度（$\rm bootstrapping$）法无法像一般梯度法那样稳定收敛。但正如下节将讲述的那样，在使用诸如线性函数这类重要的估值函数的情况中其确实可保证收敛。而且这类方法也确实因某些优势而受到更多的青睐。优势一是这类方法往往学习得非常快（参考$6、7$章）。其二是它们可直接在线地并持续不断地学习（不用等到回合结束才开始学习）。而这个特性使得该方法适用于连续型问题并借此发挥了计算优势。一个使用半梯度法的范例是半梯度$\rm TD(0)$法，其目标值为$U_t\doteq R_{t+1}+\gamma\hat v(S_{t+1},\rm w)$。下页的方框中我们给出了该方法的完整伪代码。
...
​	“批量归一化”（batch normalization）是另一种简化深度$\rm ANN$训练的技巧。一个不算新的共识是将网络的输入先归一化（normalization）再训练可以简化$\rm ANN$的学习---如将每个输入变量调整成一个有着零均值和单位方差的值。在$\rm ANN$训练中，批量归一化就是将网络某层的输出在传递给下一层前，先对输出进行归一化处理。在2015年Ioffe和Szegedy通过利用训练样本子集（mini-batches）的统计数据（statistics）来对层与层间的信号进行归一化处理，进而提升深度$\rm ANN$的学习效率。
...
​	那么$\rm LSTD$有多复杂？根据上述公式，其复杂度似乎随着$t$的增加而增加。根据（9.19）的两个公式，使用之前所述的技巧（比如第2章）可以递增地计算$\rm LSTD$，所以每一步的更新仅需固定时长。虽然如此，$\widehat {\rm A}_t$的更新离不开外积（列向量乘以行向量），所以这是一个矩阵式的更新；其计算复杂度还是会有$O(d^2)$，自然为了存储$\widehat {\rm A}_t$矩阵其所需的内存也会有$O(d^2)$。
...
>
> ​		$S\leftarrow S'$；$\rm x \leftarrow \rm x'$
>
> ​	直到$S'$为最终状态
...
​	当然对于半梯度$\rm TD$法，$O(d^2)$的开销依然远大于$O(d)$。为了利用$\rm LSTD$来高效地使用数据，是否值得承受如此大的计算开销呢？答案取决于$d$的大小、学习速度的重要性以及系统其他部分的开销。$\rm LSTD$的另一个优点是不需要步长参数[^5]，但该优势往往被过度夸大了。因为其虽不需要步长参数，却仍需要$\varepsilon$；如果选取的$\varepsilon$很小，则逆矩阵序列（ the sequence of inverses）的波动幅度会很大，反之若选择太大，则学习速率会太慢。另外，因为$\rm LSTD$没有步长参数，所以其永不遗忘（数据）。有时这种特性是我们所需要的，但在强化学习中以$\rm GPI$的方式学习任务时，因为策略$\pi$会改变，该特点反而会带来麻烦。在（强化学习的）控制应用中，典型的做法是将$\rm LSTD$和其他机制结合在一起以引入健忘机制，进而保持其不需要步长参数的优势。

[^5]: 即学习率

### 9.8	基于内存（memory）[^6]的函数拟合

​	到目前为止，我们讨论的估值拟合都是基于参数化的方式。在这种方法中, 学习算法通过调整函数形式的参数, 目的是在一个问题的整个状态空间上逼近值函数。每一次更新$s\to g$，就是学习算法通过训练样本来调整参数以减少估值误差。更新之后，该训练样本就可以被丢弃了（尽管我们可以保存下来再次使用）。当需要对某状态（我们称其为“被询状态”）估值时，函数只是简单地通过学习算法学习到的最新参数来对该状态进行估值。

​	而基于内存的函数拟合却很不一样。当有新的训练样本时，这些样本只是被简单地保存到内存中（至少会保存一个子集）却不会对参数做任何更新。然后，当某被询状态出现时，算法会从内存中调取一部分样本用以对该状态进行估值。这种方法有时被称为“延迟学习”（lazy learning），因为只有等到被要求提供一个输出时算法（system）才会去处理训练样本。

​	基于内存的函数拟合是“无参”法（nonparametric）领域内的主力军。和带参方法不同的是，其估值的方式不会像带参法那样受制于某种固定的函数（如线性函数，多项式）。求估值完全由训练样本本身来决定---通过结合样本的均值，可对被询样本给出一个估值。随着内存中样本的不断积累，对于任意目标函数，这种无参法的估值精度会越来越好。

​	基于内存的方法（译者：以下简称内存法）中也有许多不同类型。对于训练样本，不同的类型会有不同的存储、选择和处理方式。这里我们讨论一种被称为“局部学习”（local-learning）的方法。其通过被询状态的就近状态来协助估值，即算法会从内存中取出一部分和被询状态相关性最高的状态集。而这个相关性往往是通过状态间的距离来度量的：训练集中状态距离被询状态越近，意味着相关性越高，而距离的度量有很多种方式。最后在有了被询状态的估值后，整套就近状态的估值就没用了。

​	一个最简单的内存法是“最近邻”算法（nearest neighbor）。其简单地将训练样本集中距离被询状态最近的状态值作为被询状态的估值。换言之，若被询状态是$s$，$s'\to g$代表训练集中距离$s$最近的状态是$s'$,其值为$g$,则算法会返回$g$作为被询状态$s$的估值。比这个算法稍微复杂点的是通过对就近的几个状态值做“加权平均”（weighted average），将计算得到的平均值作为被询状态的估值。距离被询状态越远，则相应的权重越小。在“局部加权回归”（locally weighted regression）中也有类似的做法---寻找一个匹配最近邻样本集的平面。（如式9.1那样）寻找的过程就是通过一个参数化的拟合法来最小化权重误差实现。其中的参数权重取决于到被寻状态的距离，而被询状态在该平面上的值就是最终的估值。估值完成后，这个局部拟合的平面也就没有用了。

​	相对于参数化的方法，无参法的一个优势在于其并不拘泥于某种预先设计的函数形式。这意味着随着样本的不断累积，无参法的精度会不断提升。而因为一些其他特点，基于内存的局部估值法也很适用于强化学习。正如8.6节所述，基于轨迹的样本在强化学习中是如此的重要，因局部估值法可通过从真实或模拟的轨迹中寻找局部近邻状态（或状态操作对）来估值。而之所以不做全局的拟合是因为很多状态可能不会（至少是不太会）再碰到第二次了。另外，相较于带参法那种为了照顾全局拟合的递增式参数调整，（无参的）内存法使得智能体能更直接地影响当下状态的近邻状态的估值。

​	避免全局拟合的另一个好处是解决了维数灾难。例如，对于一个有着$k$维的状态空间，基于表格法的全局拟合需要指数级为$k$的内存空间。另一方面，若用内存法来存储样本，则所需的内存空间仅正比于$k$ 。假设总共有$n$个样本，则存储空间可表述为$n$的线性关系，根本达不到$k$或$n$指数级这种规模。当然，一个关键问题在于内存法可否及时给出某被询状态的估值，这对于智能体很重要。同时我们也关心其反馈速度随着内存增长会递减到什么程度。而一般在一个大数据集中寻找最近邻是不太现实的。

​	内存法的拥护者们研发出了一些加速寻找最近邻的方法 。如可以采用并行计算机或有特殊用途的硬件设备；也可以用一种特制的数据结构来存储数据。根据相关研究提出了一种被称为$k$维树的数据结构---其通过在二叉树下递归地展开$k$维空间作为子节点来存储数据。根据数据量及其在状态空间中的分布，运用了$k$维树的近邻搜索可快速地排除（不相关的）的状态空间。这使得在某些问题中需要消耗大量时间的传统搜索现在变的可行了。

​	另外局部加权回归也需要一些加速方法用在局部回归的运算上，因为每个被询状态都需要做该计算。针对这个，研究者们也做了诸多尝试。如可以通过遗忘一些输入（entries）进而将数据集的规模控制在一定范围内。在本章最后的参考文献和历史备注中给出了部分关于在强化学习中运用内存法学习的论文资料。

[^6]: 这里的memory可以翻译成记忆或者内存，但鉴于后文的描述，译者认为这里翻译成内存更好。而根据存储方式的不同，内存可能指rom，也可能指ram。

### 9.9 基于核的函数拟合

​	如之前讨论的加权平均或局部加权回归这类内存法都需要做一件事---就是在给定被询状态$s$和相关状态$s'$间的距离，需要将权重赋予给相关状态，即$s'\to g$。我们将执行赋值这一功能的函数成为”核函数“（kernel function），简称为核。如在加权平均或局部加权回归法中，其核函数$k: \mathbb R\to \mathbb R$会将权重赋予状态间的距离。更一般的情况是，权重的计算可以依赖状态间的其他相似度量指标，而不一定非得是距离。在这里，$k:\mathcal S\times \mathcal S \to \mathbb R$，根据$s'$对被询状态$s$的影响，其会将权重$k(s,s')$赋给相应的$s'$。

​	有一点不同的是，$k(s,s')$是一种衡量状态$s'$到$s$的泛化强度的度量。数值上核函数表达了状态和状态间彼此知识的相关性。作为示例，我们可以参考图9.11中tile coding的泛化强度就是通过核函数构造了一个均匀而又非对称的不同偏移的tile来完成的。虽然实际操作中tile coding并没有显式得调用核函数，但确实调用了。实际上正如下文所述，线性函数拟合的泛化强度总是能用核函数来表示。

​	“核回归”（Kernel Regression）是内存法的一种，其会先对存储在内存中样本的目标值做加权平均，然后将其作为核赋值给被询状态。若$\mathcal D$是存储的样本集，而$g(s')$表示某个样本状态$s'$的目标值，则通过核回归来拟合目标函数的公式可表示为（这里是$\mathcal D$中的估值函数）：
$$
\hat v(s,\mathcal D)=\sum_{s'\in \mathcal D}k(s,s')g(s'). \tag{9.22}
$$
​	上式的这个加权平均法是个特例，因为$k(s,s')$是非零的。而这只会发生在$s$和$s'$足够接近时---因而不需要对$\mathcal D$中所有样本计算求和。

​	在9.5.5中用于$\rm RBF$函数拟合的高斯径向基函数（$\rm RBF$）就是一个常用的核函数。该方法中所谓的$\rm RBF$就是特征。该特征的中心和宽度从一开始就确定了---这里的中心就是大多数样本应该会座落在的地方，或者其会随着学习的进行而以某种方式做一定的调整。除非我们使用的是会调整中心和宽度的方法，否则该法就是一个含有线性参数的方法，其中每个$\rm RBF$的权重就是参数，并通过（半）随机梯度下降法来学习。整个拟合过程是通过预先设定的$\rm RBF$的线性组合来完成的。而带有$\rm RBF$的核回归则有$\rm 2$个不同点。其一，它是内存法中的一种---用于存储$\rm RBF$中心的状态是从被存储的样本集中获取的；其二，其是无参的---因为没有参数可学；对于某个具体的被询状态，其反馈正如式$\rm 9.22$所示。

​	核回归在实际操作中当然会有很多问题，但这超出了本书的讨论范畴。然而，任何以特征向量${\rm x(s)}=(x_1(s),x_2(s),...,x_d(s))^{\sf T}$为表征的带参线性回归法（如9.4节所述）都可以映射（recast）为带有$k(s,s')$的核回归，其中$k(s,s')$是状态$s$和$s'$的特征向量的内积，其公式为：
$$
k(s,s')=\rm x(s)^{\sf T}x(s') .\tag{9.23}
$$
​	相较于传统的带参的线性函数拟合，带有核函数的核回归其实也有类似的拟合结果。只要保证其确实使用了这些特征向量并且采用了同样的训练数据。

​	这里我们直接给出了（关于核回归）的重要含义，并跳过了相关的数学证明。若读者感兴趣，可在任何讲解现代机器学习的书中找到相关证明，如Bishop($2006$)。相较于构建用于线性函数拟合器的特征，我们可以取而代之地直接构建了一组核函数，这样做就完全不用理会特征向量了。虽然不是所有的核函数都可以表示为特征向量间的内积（如式$9.23$），但与相对应的带参方法比较而言，其有着无可比拟的优势。对于有很多组特征向量的情况，式（$9.23$）给出了一个紧凑的函数表达式---在估值过程中其不需要在$d$维特征空间做任何的计算。在这类情况中，相比直接使用特征向量来表示状态的带参线性拟合法，核回归要简单得多。而这就是所谓的“核技巧”，其出现使得我们可以高效得于高度膨胀的特征空间中进行计算，而实际上只有作用在被存储的训练样本集上时该方法才是可行的。可以说核技巧是很多机器学习方法的根基，而相关研究也表明了其对强化学习也是有一定作用的。

### 9.10 深入在策学习：兴趣和焦点

 	本章中，到目前为止我们对所有算法中提及的所有状态皆给予了相同的权重，即我们认为每个状态都一样重要。然而在某些情况下，我们只对部分状态感兴趣。如在带有折扣系数的回合制问题中，相较于回合中后期的状态估值，我们兴许对初期状态的确切估值会更有兴趣，因为折扣系数使得初始状态对回馈不再敏感。另一种情况可能是，当需要学习一个操作值函数（action-value funciton）时，我们可能不会对表现差的操作感兴趣，因为其比贪婪操作（greedy action）的分值要低很多。鉴于函数拟合的资源是有限的，所以若我们能将资源应用于更有目标的地方，那么算法的表现势必会更好。

​	我们之所以认为每个状态都一样重要的一个原因是我们的估值更新是基于在策分布的---在半梯度法下，该分布已有夯实的理论保证结果收敛（原话是：stronger theoretical results ）。还记得什么是在策分布吗？当一个分布满足其是在$\rm MDP$中实际遇到的状态分布且服从目标策略，那么其被称为在策分布。现在我们将对该概念做更进一步的延伸。相较于之前在一个$\rm MDP$中只有一个在策分布，现在我们将拥有许多个在策分布。它们都是基于（回合）轨迹的状态分布，且服从目标策略。但它们会因轨迹的不同而有所不同---某种意义上，就是说会因为不同的轨迹而被初始化为不同的分布。

​	现在让我们引入一些新的概念。首先是个被称作“兴趣”的随机变量$I_t$。这是个非负数标量，代表了在时间$t$我们到底在多大程度上对当前遇到的状态（或者状态操作对）的精确估值感兴趣。若我们一点都不在意，则该兴趣值应该为零；若我们完全在意，则取值为一（虽然理论上来说可取任意非负数）。该兴趣值可通过任意因果关系（causal way）来设置。如，可以基于一直到时间$t$所产生的轨迹或者学习到的参数来设置。 在这里式(9.1)中$\rm \overline {VE}$的分布$\mu$就是个以兴趣为权重，基于实际遇到的状态而又服从目标策略的分布。另一个需要引入的非负数随机标量式变量是“焦点”（emphasis）$M_t$。用此标量乘以学习后的更新值能达到聚焦或者不聚焦在时间$t$的学习结果。相对于式(9.15)，对于一般的$n$步学习规则现在变为：

​	
$$
{\rm w}_{t+n}={\rm w}_{t+n-1}+\alpha M_t[G_{t:t+n}-\hat v(S_t,{\rm w}_{t+n-1})]\nabla\hat v(S_t,{\rm w}_{t+n-1}),\ 0\leq t <T, \tag{9.24}
$$
根据式(9.16)给出的$n$步回报，变量焦点可以根据变量兴趣递归地定义为：
$$
M_t=I_t+\gamma^nM_{t-n},\ 0\leq t<T, \tag{9.25}
$$
这里对于所有$t<0$，有$M_t \doteq 0$。该式同样考虑了采用$\rm MC$法的情况。在$\rm MC$法中，$G_{t:t+n}=G_t$，只有回合结束后才会进行更新，其中有$n=T-t$且$M_t=I_t$。

​	示例$9.4$阐述了为何兴趣和焦点可以获得更精确的估值。
...
> ------
>
> ​	为了展示使用兴趣和焦点的潜在好处，请看下面这个有着$4$个状态的马尔科夫过程：
>
> ![Example 9.4](.\img\Example 9.4.JPG)
>
> 该回合从最左边的状态开始，然后一个状态接着一个状态从左向右进行着。在到达终点前，每一时步的回馈是$+1$。所以如上图下方所示，状态一的值应该是$4$，状态二的应该是$3$并依此类推。 注意到这些都是状态的真值，而受到参数的限制的估值法只能做到“估”值。这里我们用两个元素来组成参数向量${\rm w}=(w_1,w_2)^\sf T$，见上图代表状态的圆圈。这里前两个状态的估值仅受$w_1$（译者：就是图中的$\theta_1$，后面的$w_2$同理）的影响，所以尽管真值不一样，两者的估值却应该是一样的。类似的，仅受$w_2$影响的后两个状态的估值也应该是一样的，虽然实际上并不同。这里假设我们只对最左边的状态感兴趣；那我们会将该状态的兴趣值设为$1$而将其他状态的兴趣值设为$0$。
>
> ​	首先我们来考虑下在本例中使用蒙特卡洛法的情况。本章之前所述的所有没有考虑兴趣和焦点的算法（参考式($9.7$)以及（原版）第$165$页的框中示例）在(学习率递减的情况下)最后会使得参数收敛至$w_\infty =(3.5,1.5)$---即这种情况下，我们唯一感兴趣的状态一的估值被赋予了$3.5$（即，介于状态一和状态二真值之间）。而若使用本节所述的兴趣和焦点，则算法最后可以丝毫不差地学到真值；最后$w_1$会收敛至4而$w_2$则至始至终不会有任何变化，因为除了最左边的状态，其他所有状态的焦点值为$0$。
>
> ​	现在来看看应用两步半梯度$\rm TD$法的情况。若基于之前没有兴趣和焦点的方法（式$(9.15)$和式$(9.16)$及（原版）第$171$页的框中示例）则最终的收敛结果是$w_\infty =(3.5,1.5)$，而带有兴趣和焦点的则会最终收敛至$w_\infty =(4,2)$。后者给出了状态一和状态三（用于状态一的bootstrap）的准确估值，而至始至终状态二和状态四都没有任何更新。

> ------

### 9.11 总结

​	为了适用于人工智能或大型工程应用，强化学习系统必须有足够的“泛化”能力。为此，我们可简单地将每次（在环境交互中）更新的数据作为训练样本，并将其和任何已存在的监督学习方法相结合来进行函数拟合。
...
​	为了找到合适的权重向量，最流行的方法是“随机梯度下降”($\rm SGD$)类的方法。本章我们着重讨论了使用“固定策略”（fixed policy）的“在策略”的强化学习案例（策略更新或预测）；很自然地，对该类案例的策略更新可选择是“$n$步半梯度$TD$法”（ n-step semi-gradient TD）。其中当$n=\infty$或$n=1$时，引出了$2$种特殊算法，梯度$\rm MC$和半梯度$\rm TD(0)$.半梯度$\rm TD$法不是真正的梯度法。因为在这类bootstrapping法（包括$\rm DP$）中权重向量在更新目标中有出现，但该处的权重在计算梯度时却没有被考虑---因此这类算法被称为“半”梯度法。基于此，传统的$\rm SGD$法结论并不适用于半梯度法。
...
**9.5.5** 自从 Broomhead和Lowe(1988)将径向基函数（RBF）和神经网络联系起来后，在函数拟合中使用该方法就受到了广泛关注。 Powell (1987)对早期$\rm RBF$的使用作出了评估，而Poggio和Girosi(1989, 1990)则广泛发展并应用了这种方法。
...
**9.7**	$\rm LSTD$最先由 Bradtke和Barto (参考：Bradtke, 1993, 1994; Bradtke和Barto, 1996; Bradtke, Ydstie,和Barto, 1994)提出，随后Boyan (1999，2002)、 Nedi´ c和Bertsekas (2003)对其进行了深入研究。关于逆矩阵的增量更新的研究资料可追溯至1949年 (Sherman和Morrison, 1949)。 Lagoudakis and Parr (2003)介绍了一种基于最小二乘法的拓展，用以研究控制类问题。



**9.8**	我们所讨论的基于内存的函数拟合在很大程度上是基于“局部权重的学习”---由 Atkeson, Moore, 和Schaal (1997)提出。Atkeson (1992)研究了在一个基于内存的机器中使用局部加权回归的情况，并给出了大量的关于该想法的历史资料。 受此影响，Stanfill和Waltz (1986)认为基于内存的方法可能对人工智能很重要，尤其是在当时已经有了采用并行架构的产品（如Connection Machine）背景下。 Baird和Klopf (1993)在平衡木任务重采用了基于内存（用以函数拟合）的Q学习法，并取得了不错的成果。为了习得一个系统模型，Schaal和Atkeson (1994) 在一个机器人（robot juggling）控制类任务中采用了局部加权回归。 以平衡木任务为平台，Ping (1995)尝试了使用不同的近邻算法来拟合（状态）值函数、策略和环境的模型。 利用局部加权线性回归来学习估值函数，Tadepalli和Ok (1996)在一模拟自动导引车任务中取得了振奋人心的成果。 在一些模式识别任务中，Bottou和Vapnik (1996) 指出了部分局部学习算法有着远超非局部算法的效率，并对局部学习算法在泛化中的影响做了说明。

​	Bentley (1975)提出了$\rm k-d$树，并指出当采用最近邻搜索$\rm n$条记录时，运行开销为${\rm O(log }\,n)$。 Friedman、Bentley和Finkel (1977) 对采用$\rm k-d$树的最近邻搜索做了梳理。 Omohundro (1987) 讨论了关于使用诸如$\rm k-d$树这类分层数据结构的效率。 为了提升效率，Moore、Schneider和Deng(1997)介绍了将$\rm k-d$树用于局部加权回归的方法。 



**9.9**  核回归的历史需追溯到Aizerman、Braverman和Rozonoer (1964)的文献“论势函数方法”（ method of potential functions）。They likened the data to point electric charges of various signs and magnitudes distributed over space. The resulting electric potential over space produced by summing the potentials of the point charges corresponded to the interpolated surface. In this analogy, the kernel function is the potential of a point charge, which falls off as the reciprocal of the distance from the charge.（译者：这里一段鉴于译者阅历有限，不知如何翻译）。 Connell和Utgoff (1987)在平衡木任务中应用了actor–critic法，而其中的评价器（critic）就是通过结合了反距离加权的核回归来学习的估值函数。在核回归开始在机器学习领域流行之前，上述作者们还没开始用“核”这个词来表示该方法，而是用的‘Shepard’s method“这个术语(Shepard,1968)。另一些关于在强化学习中使用基于核方法的文献可参考自Ormoneit和Sen (2002)、Dietterich和Wang (2002)、Xu、Xie,、Hu、Nu和Lu (2005)、Taylor和Parr (2009)、Barreto、Precup和Pineau (2011)和Bhat、Farias和Moallemi (2012)。



**9.10** 请参考第$11.8$节关于$\rm Emphatic-TD$法的参考文献。
...
​	Holland(1986)的分类系统运用了可选特征匹配技术（selective feature-match technique）来评估所有状态-动作对。每个分类器用以匹配一个特定的状态子集，该子集的部分特征有特定的值而其他的特征值则可取任意值（犹如扑克中的百搭牌“wild cards”）。然后在典型的状态聚合法中使用这些子集来进行函数拟合。Holland的想法是通过遗传算法来改进一组分类器。这样在经过不断地积累后分类器会变成一个有用的动作值函数。Holland的想法影响到了早期研究强化学习的作者，但我们这里聚焦于其他不同的函数拟合法。原因是分类器用于函数拟合有诸多局限。首先，其属于状态整合法。该类方法的局限使得其无法高效地缩放（scaling）和展示光滑的函数。另外，分类器在配对时仅依赖于平行于特征轴的（are parallel to the feature axes）聚合边界。如果要说传统的分类器系统最主要的局限的话，那么很可能是在于其分类器是从遗传算法（一种调优法）学习而来的。正如第1章所述，通过学习而不是调优法，agent能发现更多关于如何学习的细节信息（there is available during learning much more detailed information about how to learn than can be used by evolutionary methods.）。这个观点指引我们转而使用监督学习，尤其是使用其中的梯度下降和神经网络来完成强化学习任务。Holland和我们采用的方法会有着如此般的不同并出人意料。因为Holland提出其方法的时期正值神经网络被普遍认为因计算能力太差而表现太弱的时候。而我们开展相关工作时则正值该传统方法暴露出了诸多问题的时候。（应该说）依然很有可能将这些不同的方法整合到一起。
